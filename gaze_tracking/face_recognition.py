...
def login(flag):
    ...
    layout = [
        [sg.Text("Window Main", size=(60, 1), justification="center")],
    ...
    # names = ['None', 'Adeel', 'Paula', 'Ilza', 'Z', 'W']
    ...
    while True:
        ...
        if tems<1.5 and tems>1.2:
            starttimes(text)
        # print("counter " + str(counter) +" "+str(counterl))
        ret, img =cam.read()
        gaze.refresh(img)
        ...
        if values["-THRESH-"]:
            ...
            img = cv2.threshold(
                img, values["-THRESH SLIDER-"], 255, cv2.THRESH_BINARY
            )[1]
        elif values["-CANNY-"]:
            img = cv2.Canny(
                img, values["-CANNY SLIDER A-"], values["-CANNY SLIDER B-"]
            )
        elif values["-BLUR-"]:
        ...
        # if registerb:
        #     cv2.putText(img, "Registration Process", (50, 50), cv2.FONT_HERSHEY_DUPLEX, 1.5, (147, 58, 31), 2)
        # elif loginb:
        #     cv2.putText(img, "Login Process", (50, 50), cv2.FONT_HERSHEY_DUPLEX, 1.5, (147, 58, 31), 2)
        ...
        cv2.putText(img, str(text), (90, 60), cv2.FONT_HERSHEY_DUPLEX, 1.6, (147, 58, 31), 2)
        cv2.putText(img, str(texttwo), (90, 30), cv2.FONT_HERSHEY_DUPLEX, 1.6, (147, 58, 31), 2)
